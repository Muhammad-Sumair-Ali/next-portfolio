...
export async function PUT(
  ...
  try {
    ...
    const updateData: any = { title, description, tags, demoLink, githubLink, isPinned };
...
export async function PATCH(
  ...
  try {
    ...
    const data = await req.json();
    const updatedProject = await Project.findByIdAndUpdate(projectId, { $set: data }, { new: true });
